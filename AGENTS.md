...
- **Do not append `Co-authored-by: Cursor <cursoragent@cursor.com>` (or any other agent co-author trailer) to commit messages.** This repo is published under `Vickyjohnson2004` as the sole author for Context Protocol grant attribution. Past commits had the trailer stripped via filter-branch on 2026-05-09; future commits must not reintroduce it.
